...
def plot_XR(X, R, T, tau_x, ax, n, d='max'):
    if d is None:
        _, _, D = X.shape
        ...
        var = np.std(R[:, n, :], axis=0)
        plt.plot(var)
        plt.show()
        assert False
        X_proj = X[:, n, var.argmax()]
        R_proj = R[:, n, var.argmax()]
    else:
        X_proj = X[:, n, d]
        R_proj = R[:, n, d]


    ax.plot(T / tau_x, X_proj, C=CX, label=rf'$x_1$')
...
def plot_evo_one(X, R, A, T, tau_x, exp):
    ...
    if exp == 'lsc':
        ...
        plt.annotate(s=r'$\tau_A$', fontsize=14, xy=(X+.2, Y), xytext=(X,Y+.0001), c=c)
...
def plot_exp_one(exp, save=False):
    ...
    plt.gcf().subplots_adjust(left=0.05, right=.99, bottom=0.13)
    if save:
